...
export function DiscoverContent() {
	...
	return (
		<div className="flex-1 min-h-screen">
			...
			<div className="border-b border-gray-700/30 bg-background/95 backdrop-blur-md px-6 py-4">
			...
			<div className="p-6">
				...
				<div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
					...
					<div className="lg:col-span-3 space-y-6">
						{/* Featured Collection Banner */}
						<div className="bg-gray-900/50 border border-gray-700/30 rounded-lg overflow-hidden hover:border-blue-500/50 transition-colors group">
							{/* Banner Image */}
								...
								<div className="absolute inset-0 flex flex-col justify-between p-4">
									<div>
										<div className="flex items-center gap-2 mb-1">
											<h2 className="text-xl font-bold text-white">{FEATURED_COLLECTION_BANNER.name}</h2>
											{FEATURED_COLLECTION_BANNER.verified && (
												<span className="text-blue-400 text-sm">✓</span>
											)}
										</div>
										<p className="text-xs text-white/80">{FEATURED_COLLECTION_BANNER.creator}</p>
									</div>

									{/* Stats - Compact */}
									<div className="grid grid-cols-2 gap-3 text-white text-xs">
										<div>
											<p className="text-white/60 text-xs mb-0.5">FLOOR</p>
											<p className="font-bold text-sm">{FEATURED_COLLECTION_BANNER.floorPrice}</p>
										</div>
										<div>
											<p className="text-white/60 text-xs mb-0.5">VOLUME</p>
							...
							<div className="bg-gray-950/80 border-t border-gray-700/30 px-3 py-2 flex items-center justify-between">
								<button
									onClick={goToPrevImage}
									className="p-1 rounded-lg bg-gray-800/50 hover:bg-gray-700 transition-colors"
								>
							...
							<Carousel className="w-full">
								<CarouselContent className="-ml-2 md:-ml-4">
									{TRENDING_TOKENS.map((token) => (
										...
										>
											<div className="bg-gray-900/50 border border-gray-700/50 rounded-lg p-4 hover:border-blue-500/50 transition-all hover:shadow-lg hover:shadow-blue-500/10 h-full">
												{/* Token Header */}
												...
												<div className="space-y-2 mb-4">
													<p className="font-bold text-lg text-white">{token.price}</p>
													<p
												...
												{/* Mini Trend Chart */}
												<div className="h-12 bg-gray-800/50 rounded flex items-end gap-0.5 p-1">
													{token.trend.map((value, i) => (
														<div
															key={i}
															className="flex-1 bg-linear-to-t from-green-500 to-green-400 rounded-sm"
															style={{
						...
						<section className="space-y-4">
							<div>
								...
								<p className="text-sm text-gray-400">This week&apos;s curated collections</p>
							</div>
							...
							<Carousel className="w-full">
								<CarouselContent className="-ml-2 md:-ml-4">
									{FEATURED_COLLECTIONS.map((collection) => (
										...
										>
											<div className="bg-gray-900/50 border border-gray-700/50 rounded-lg overflow-hidden hover:border-blue-500/50 transition-all hover:shadow-lg hover:shadow-blue-500/10 h-full">
												{/* Image */}
												...
												{/* Info */}
												<div className="p-4">
													<p className="text-xs text-gray-400 font-medium mb-2">{collection.category}</p>
													<h3 className="font-semibold text-white mb-3 line-clamp-2">{collection.name}</h3>
					...
					<div className="lg:col-span-1 space-y-4">
						<div>
							<h2 className="text-2xl font-bold text-white mb-1">NFTs & Tokens</h2>
							<p className="text-sm text-gray-400">Popular collections</p>
						</div>

						{/* Collections List - Large Scrollable */}
						<div className="bg-gray-900/50 border border-gray-700/30 rounded-lg h-full min-h-96 overflow-y-auto space-y-2 p-3">
							{NFT_COLLECTIONS.map((collection) => (
								<div
									key={collection.id}
									className="bg-gray-900/50 border border-gray-700/30 rounded-lg p-3 hover:border-blue-500/50 transition-all hover:bg-gray-900/70 cursor-pointer group"
								>
										...
										<div className="flex items-center gap-2 flex-1 min-w-0">
											...
											<div className="min-w-0">
												<div className="flex items-center gap-1">
													...
													{collection.verified && <span className="text-blue-400 text-xs">✓</span>}
												</div>
